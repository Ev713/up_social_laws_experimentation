...
class SocialLawRobustnessChecker(engines.engine.Engine, mixins.OneshotPlannerMixin):
    ...
    def __init__(self, planner: OneshotPlannerMixin = None, robustness_verifier_name: str = None,
                 save_pddl_prefix=None):
    ...
    def is_single_agent_solvable(self, problem: MultiAgentProblem) -> bool:
        for agent in problem.agents:
            ...
            if self._save_pddl_prefix is not None:
                ...
                w.write_domain(
                    up_social_laws.name_separator.join([self._save_pddl_prefix, "sap", agent.name, "domain.pddl"]))
                w.write_problem(
                    up_social_laws.name_separator.join([self._save_pddl_prefix, "sap", agent.name, "problem.pddl"]))
...
### #           problem_kind=problem.kind,
    ...
    def is_robust(self, problem: MultiAgentProblemWithWaitfor) -> SocialLawRobustnessResult:
        ...
        if result.counter_example is not None:
            assert (result.status in [SocialLawRobustnessStatus.NON_ROBUST_MULTI_AGENT_FAIL,
