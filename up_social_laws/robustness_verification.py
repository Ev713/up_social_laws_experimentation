...
class RobustnessVerifier(engines.engine.Engine, CompilerMixin):
    ...
    def get_action_preconditions(self, problem: MultiAgentProblemWithWaitfor, agent: Agent, action: Action, fail: bool,
        ...
        assert fail or wait
        wait_precs = problem.waitfor.get_preconditions_wait(agent.name, action.name)
        str_wait_precs = [str(prec) for prec in wait_precs]
        if wait and not fail:
            return wait_precs
        else:
            precs = []
            for prec in action.preconditions:
                if wait or not prec in wait_precs:
                    precs.append(prec)
...
env.factory.add_engine('SimpleNumericRobustnessVerifier', __name__,
                       'SimpleNumericRobustnessVerifier')
