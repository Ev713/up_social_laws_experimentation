...
def state_to_dict(state):
    ...
    keys = [k for k in str_state.replace(': true, ', '@').replace(': false, ', '@').split('@')]
    vals = []
...
def simulate(problem, ma=False, print_state=False):
...
if __name__ == '__main__':
    zenotravel_add_sociallaw(get_zenotravel('pfile3'))
